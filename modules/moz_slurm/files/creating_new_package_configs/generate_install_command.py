...
cmd2 = "diff /tmp/output/base_bom.txt /tmp/output/final_bom.txt  | grep -E 'cuda|nvidia' | grep -v '%s'" % metapackage_to_install
result = subprocess.run(cmd2, stdout=subprocess.PIPE, encoding='UTF8', shell=True)

output = "# packages installed by metapackage %s at version %s\n\n" % (metapackage_to_install, metapackage_version)
